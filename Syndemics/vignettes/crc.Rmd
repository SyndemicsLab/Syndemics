  ...
  for (s in subgroup) {
    for (i in unique_years) {
      ...
      results[[as.character(i)]] <- data.frame(
        ...
        LCI_Estimated = mean(estimates) - 1.96 * sd(estimates),
        UCI_Estimated = mean(estimates) + 1.96 * sd(estimates),
        CRCMin = min(estimates),
        CRCMax = max(estimates),
        CRC_Total = mean(estimates) + known,
        LCI_Total = mean(estimates) + known - 1.96 * sd(estimates),
        UCI_Total = mean(estimates) + known + 1.96 * sd(estimates)
      )
