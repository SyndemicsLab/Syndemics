...
compare_moud <- function(old_path, new_path) {
    ...
    count_plot <- ggplot2::ggplot(
        combined,
        ggplot2::aes(x = date, y = N_ID, color = version, linetype = version)
    ) +
    ...
    diff_df <- dplyr::left_join(
        old_df2,
        new_df2,
        by = c("treatment", "month", "year")
    ) %>%
        dplyr::mutate(
    ...
    difference_plot <- ggplot2::ggplot(
        diff_df,
        ggplot2::aes(x = date, y = difference, fill = difference > 0)
    ) +
