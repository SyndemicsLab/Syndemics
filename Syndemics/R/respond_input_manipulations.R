...
DSA <- function(data, filter, cycle, pct_change) {
    ...
    for (c in cycle) {
        DT <- data[
            initial_block == filter,
            paste0("to_", filter, "_cycle", c) := get(paste0(
                "to_",
                filter,
                "_cycle",
                c
            ...
            paste0("to_corresponding_post_trt_cycle", c) := 1 -
                get(paste0("to_", filter, "_cycle", c))
        ]
    }
    return(DT)
}
...
change_agegrp_chunk <- function(
    ...
    if (!cols %in% names(DT)) {
        stop("Column names in 'cols' do not match names in 'data'")
    }
    if (missing(grouping)) {
        grouping <- names(DT)[!c(names(DT) %in% c(cols, "agegrp"))]
    }

    DT <- DT[, `:=`(
        age_min = as.integer(sub("_(.*)", "", agegrp)),
    ...
    expanded <- merge(
        ...
        agegrp = cut(
            age,
            seq(9, 100, size.out),
            labels = paste0(seq(10, 98, size.out), "_", seq(11, 99, size.out))
        ),
        age = NULL
    )]
    if (transformation == "sum") {
        out <- expanded[,
            (cols) := lapply(cols, function(x) get(x) / size.in),
        ][,
            (cols) := lapply(cols, function(x) sum(get(x))),
            by = c("agegrp", "block", "oud", "sex")
        ]
        return(unique(out))
...
new_block <- function(data, names, column = "block") {
    if (!column %in% names(data)) {
        stop(paste(column, "is not in original data"))
    }
    data <- as.data.table(data)
    data_list <- list()
...
replace_vals <- function(data, column = "block", x, y, a, b) {
    ...
    for (i in seq_along(a)) {
        data[, (x) := as.numeric(get(x))][
            get(column) == a[i],
            (x) := data[get(column) == b[i], y, with = FALSE][[1]]
        ]
    }
