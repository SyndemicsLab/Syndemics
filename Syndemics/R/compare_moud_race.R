compare_moud_race <- function(old_path, new_path) {
  N_ID <- treatment <- new_count <- old_count <- difference <- final_re <- NULL
  ...
  # plot 1: time series of MOUD counts by treatment and race
  count_plot <- ggplot2::ggplot(combined, ggplot2::aes(x = date, y = N_ID, color = version, linetype = version)) +
    ggplot2::geom_line(linewidth = 1) +
    ggplot2::facet_wrap(~ treatment + final_re, scales = "free_y") +
    ggplot2::labs(
      title = "Monthly MOUD Counts by Treatment Type and Race: Old vs New Dataset",
      x = "Date",
      y = "MOUD Count",
      color = "Version",
      linetype = "Version"
  ...
  old_df2 <- read.csv(old_path, stringsAsFactors = FALSE) %>%
    dplyr::select(treatment, month, year, final_re, N_ID) %>%
  ...
  diff_df <- dplyr::left_join(old_df2, new_df2, by = c("treatment", "month", "year", "final_re")) %>%
    dplyr::mutate(
      difference = new_count - old_count,
      date = as.Date(paste(year, month, "01", sep = "-"))
    )

  # plot 2: difference in MOUD counts by treatment and race
  difference_plot <- ggplot2::ggplot(diff_df, ggplot2::aes(x = date, y = difference, fill = difference > 0)) +
    ...
    ggplot2::labs(
      title = "Change in MOUD Counts by Treatment Type and Race: New vs Old",
      x = "Date",
